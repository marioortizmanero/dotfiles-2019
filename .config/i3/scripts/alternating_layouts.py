...
# Script taken from https://github.com/olemartinorg/i3-alternating-layout,
# on arch it needs 'python-i3-py' and 'xorg-util-macros' installed.
    ...
    while True:
        try:
            ...
            if regex.match(line):
                set_layout()
            last_line = line
        # Very ugly fix for an error
        except BrokenPipeError as e:
            f = open("errors.txt", "a")
            f.write("ERROR! ", datetime.datetime.now(), "\n", e)
            f.close()
            time.sleep(5)
            os.system("python /home/mario/.config/i3/scripts/alternating_layouts.py")
            break
